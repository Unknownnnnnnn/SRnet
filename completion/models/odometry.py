...
class Odometrynet(nn.Module):
    ...
    def forward(self, tgt, src, gt, T_gt = None, prefix="train"):
        self.pts = src.transpose(1,2).contiguous()
...
#         tgt = tgt.transpose(1,2).contiguous()
        ...
        if T_gt is None:
            ...
            print_loss = [r_err.data.cpu(), t_err.data.cpu()]
#             print(R.shape)
#             print(t.shape)
#             print(self.pts.shape)
#             print((self.pts @ R).shape)
#             print(dsadsad)
#             print(R_gt.shape)
#             print(t_gt.shape)
#             a = self.pts @ R_gt + t_gt.unsqueeze(1).repeat(1,2048,1)
#             a = (self.pts - t.unsqueeze(1).repeat(1,2048,1) )@ R.transpose(2,1)
            a = (self.pts - t.unsqueeze(1).repeat(1,2048,1) )@ R.transpose(2,1)
            return a, (r_err.mean(), t_err.mean()), loss, print_loss
