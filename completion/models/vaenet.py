...
class view_back(nn.Module):
    ...
    def forward(self,input, matrix):
        ...
        for i in range(self.view_time):
            ...
            ori_matrix = matrix[i:i+1]
            tar_input[int(i*(B/self.view_time)):int((i+1)*(B/self.view_time)),:,:] = (((ori_matrix.inverse() @ ori_input)[:,:3,:])).permute(0,2,1)
        return tar_input
class one_view(nn.Module):
    ...
    def forward(self,input):
        ...
        c_ori = F.pad(c,[0,1],'constant',1)

        view_seed = random.randint(0,2)
        if(view_seed == 0):
            a = np.eye(4)
            a[0,0] = 0.3
            a[0,3] = 5
            a[1,3] = 3
            a[2,3] = 2
            T = torch.Tensor(a).cuda(c_ori.device)
        if(view_seed == 1):
            a = np.eye(4)
            a[1,1] = 0.3
            a[0,3] = 2
        ...
        if(view_seed == 2):
            ...
            a[2,3] = 2
            T = torch.Tensor(a).cuda(c_ori.device)
        c_tar = (T @ c_ori.permute(0,2,1)).permute(0,2,1) # 2,2048,4
        c_change = c_tar[:,:,:3]
        return c_change.permute(0,2,1),T
...
class Random_view(nn.Module):
    ...
    def forward(self, input):
        ...
        t_tar = torch.zeros((self.view_time,4,4)).cuda()
        for i in range(self.view_time):
            c_tar[i*B:(i+1)*B,:,:],t_tar[i] = self.view_once(input)
        return c_tar,t_tar
...
class Model(nn.Module):
    def __init__(self, args, size_z=128, global_feature_size=1024):
        ...
        self.posterior_infer3 = Linear_ResBlock(input_size=global_feature_size, output_size=global_feature_size)
        self.posterior_infer4 = Linear_ResBlock(input_size=global_feature_size, output_size=size_z * 2)
...
#         print(x.shape)
        ...
        ori_fine = fine.transpose(1, 2).contiguous()
        
        print('aaaa',ori_fine.shape)
        print(dsadsad)
        if prefix=="train":
            if self.distribution_loss == 'MMD':
                ...
                dl_rec = torch.distributions.kl_divergence(ori_distribution, standard_distribution)
...
#                 print(x,x.shape)
                ...
                loss6, _, _ = calc_cd(ori_fine, x, calc_f1=True)
                loss5 = torch.nn.functional.cross_entropy(view_class, label_class)
                loss1, _, _ = calc_cd(view_raw, torch.cat((ori_coarse_raw,ori_coarse_raw,ori_coarse_raw),0), calc_f1=True)
                loss2, _, _ = calc_cd(view_high, torch.cat((ori_coarse_high,ori_coarse_high,ori_coarse_high),0), calc_f1=True)
                loss3, _, _ = calc_cd(view_coarse, torch.cat((ori_coarse,ori_coarse,ori_coarse),0), calc_f1=True)
                loss4, _, _ = calc_cd(view_fine, torch.cat((ori_fine,ori_fine,ori_fine),0), calc_f1=True)
                
                loss_tar, s, f_s = calc_cd(ori_fine.permute(0,2,1) , gt,calc_f1=True)
#                 loss4, _ = calc_cd(view_fine, x, calc_f1=True)
                
#                 loss6 = nn.CrossEntropyLoss(view_class,view_gt)
            else:
                raise NotImplementedError('Only CD is supported')

            total_train_loss = loss1.mean() + loss2.mean()  + loss3.mean() + loss4.mean() + loss5.mean() + loss6.mean() * 50 +((dl_rec.mean() + dl_g.mean())) + (dl_view_0.mean() + dl_view_1.mean())
            return ori_fine.permute(0,2,1), f_s, total_train_loss
        elif prefix=="val":
            if self.eval_emd:
            ...
            return (cd_p,cd_t,f1)#{'out1': coarse_raw, 'out2': fine, 'emd': emd, 'cd_p': cd_p, 'cd_t': cd_t, 'f1': f1}
        elif prefix=="test":
            return {'result': fine}
