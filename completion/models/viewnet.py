...
def rand_rotation_matrix(deflection=1.0, randnums=None):
    ...
    return M
class view_back(nn.Module):
    def __init__(self, view_time):
        super(view_back, self).__init__()
        self.view_time = view_time
    def forward(self,input, matrix):
        input = input.permute(0,2,1)
        B,C,N = input.shape #6,2048,3
        tar_input = torch.zeros((B,N,C)).cuda()
        for i in range(self.view_time):
            ori_input = input[int(i*(B/self.view_time)):int((i+1)*(B/self.view_time)),:,:].permute(0,2,1)
            ori_input = F.pad(ori_input,[0,1],'constant',1).permute(0,2,1)
            ori_matrix = torch.inverse(matrix[i:(i+1)]).squeeze()
            tar_input[int(i*(B/self.view_time)):int((i+1)*(B/self.view_time)),:,:] = (ori_matrix @ ori_input)[:,:3,:].permute(0,2,1)
        return tar_input
class one_view(nn.Module):
    def __init__(self):
        super(one_view, self).__init__()
    def forward(self,input):
        c = input.permute(0,2,1)  # 2,2048,3
        c_ori = F.pad(c,[0,1],'constant',1).permute(0,2,1)
        
        matrix = np.random.rand(2048,4,4)
        random_matrix = rand_rotation_matrix()
        
        now_matrix = np.eye(4)
        now_matrix[:3,:3] = random_matrix
        now_matrix[:3,-1] = 0
        
        matrix[:,:3,:3] = random_matrix
        matrix[:,-1,:-1] = 0
        matrix[:,:-1,-1] = matrix[:,:-1,-1] /50.
        matrix[:,-1,-1] = 1

        C = torch.Tensor(now_matrix).cuda(c_ori.device)
        T = torch.Tensor(matrix).cuda(c_ori.device)
        c_tar = torch.matmul(T,c_ori.unsqueeze(1).permute(0,3,2,1)).squeeze()
        c_change = c_tar.permute(0,2,1)[:,:3,:]

        
        return c_change,C
        
class Random_view(nn.Module):
    def __init__(self,view_time):
        super(Random_view, self).__init__()
        self.view_time = view_time
        self.view_once = one_view()
    def forward(self, input):
        B, C, N = input.size()
        c_tar = torch.zeros((self.view_time*B,C,N)).cuda()
        t_tar = torch.zeros((self.view_time*N,4,4)).cuda()
        for i in range(self.view_time):
            c_tar[i*B:(i+1)*B,:,:],t_tar[i:(i+1),:,:] = self.view_once(input)
        return c_tar,t_tar
...
class Model(nn.Module):
    def __init__(self, args, type_, size_z=128, global_feature_size=1024):
        ...
        self.toolbox = Random_view(view_time = 3)
        self.view_back = view_back(view_time = 3)
...
#         print(x.shape)
        if(self.type != 'third'):
            B,C,N = x.size()
            view_time = 3
            num_input = x.size()[2] # (2,3,2048)
    #         print(x.shape)
            feat = self.encoder(x) #(2,1024)
            ...
            standard_distribution = torch.distributions.Normal(torch.zeros_like(q_mu), torch.ones_like(q_std))  # 标准正态分布

            q_mu_concat, q_std_concat = torch.cat((q_mu,q_mu,q_mu),0), torch.cat((q_std,q_std,q_std),0)
            q_std_concat = F.softplus(q_std_concat)
            tar_distribution = torch.distributions.Normal(q_mu_concat, q_std_concat)  #输入不完整的正态分布

            z = ori_distribution.rsample()  #(2,128)
        else:
            B,C,N = x.size()
            view_time = 3
            num_input = x.size()[2] # (2,3,2048)
            feat_x = self.encoder(x)
            feat_y = self.encoder(gather_points(gt.transpose(1, 2).contiguous(), furthest_point_sample(gt, num_input)))
        ...
        feat = feat + self.generator(z)
        coarse_raw, coarse_high, coarse, fine = self.decoder(feat, x)
        ori_coarse_raw = coarse_raw.transpose(1, 2).contiguous()
        ori_coarse_high = coarse_high.transpose(1, 2).contiguous()
        ori_coarse = coarse.transpose(1, 2).contiguous()
        ori_fine = fine.transpose(1, 2).contiguous()  #4,3,2048
        
        if(self.type == 'second'):
            trans ,matrix = self.toolbox(x)  # 6,3,2048
            view_raw = self.view_back(trans.transpose(1, 2).contiguous(),matrix)
            print(x.shape)
            print(trans.shape)
            print(view_raw.shape)
#             torch.Size([2, 3, 2048])
# torch.Size([6, 3, 2048])
# torch.Size([6, 2048, 3])
            for i in range(x.shape[0]):
                plot_single_pcd(x[i].permute(1,0).cpu().numpy().squeeze(),'./res/ori_'+'tmp_'+str(i)+'.png')
                plot_single_pcd(trans[2*i].permute(1,0).cpu().numpy().squeeze(),'./res/ori_'+'tmpp_'+str(i)+'.png')
                plot_single_pcd(view_raw[2*i].cpu().numpy().squeeze(),'./res/ori_'+'tmppp_'+str(i)+'.png')
                plot_single_pcd(gt[i].cpu().numpy().squeeze(),'./res/ori_'+'tmpppp_'+str(i)+'.png')
            print(Dsadsad)
#             for i in range(view_raw.shape[0]):
#                 plot_single_pcd(trans[i].permute(1,0).cpu().numpy().squeeze(),'./res/ori_'+'tmppp_'+str(i)+'.png')
                
#                 plot_single_pcd(view_raw[i].cpu().numpy().squeeze(),'./res/ori_'+'tmp_'+str(i)+'.png')
#             print(dsdsds)
            feat_view = self.encoder(trans) #(2,1024)
            o_x_view = self.posterior_infer2(self.posterior_infer1(feat_view))
            q_mu_view, q_std_view = torch.split(o_x_view, self.size_z, dim=1)
            q_std_view = F.softplus(q_std_view)
            view_distribution = torch.distributions.Normal(q_mu_view, q_std_view)  #输入不完整的正态分布
            z_view = view_distribution.rsample()  #(2,128)

            feat_view = feat_view + self.generator(z_view)


            view_raw, view_high, view_coarse, view_fine = self.decoder(feat_view, trans)

            # [6, 1024, 3])
            view_raw = self.view_back(view_raw.transpose(1, 2).contiguous(),matrix)
            view_high = self.view_back(view_high.transpose(1, 2).contiguous(),matrix)
            view_coarse = self.view_back(view_coarse.transpose(1, 2).contiguous(),matrix)
            view_fine = self.view_back(view_fine.transpose(1, 2).contiguous(),matrix)

            view_feat = self.encoder(view_fine.permute(0,2,1))
            view_class = self.classfication(view_feat)

            label = label.unsqueeze(1)
            label = torch.cat((label,label,label),0)
            label_class = label.squeeze().long()
    #         label_class = F.one_hot(label, num_classes=16) 
    #         print(view_class)
    #         print(label_class)

            o_x_class = self.posterior_infer2(self.posterior_infer1(view_feat[:B]))
            q_mu_class, q_std_class = torch.split(o_x_class, self.size_z, dim=1)
            q_std_class = F.softplus(q_std_class)
            class_distribution_1 = torch.distributions.Normal(q_mu_class, q_std_class)  #3*B,128

            o_x_class_ = self.posterior_infer2(self.posterior_infer1(view_feat[B:2*B]))
            q_mu_class_, q_std_class_ = torch.split(o_x_class_, self.size_z, dim=1)
            q_std_class_ = F.softplus(q_std_class_)
            class_distribution_2 = torch.distributions.Normal(q_mu_class_, q_std_class_)  #3*B,128

            o_x_class__ = self.posterior_infer2(self.posterior_infer1(view_feat[2*B:3*B]))
            q_mu_class__, q_std_class__ = torch.split(o_x_class__, self.size_z, dim=1)
            q_std_class__ = F.softplus(q_std_class__)
            class_distribution_3 = torch.distributions.Normal(q_mu_class__, q_std_class__)  #3*B,128
        ...
        if prefix=="train":
            if self.distribution_loss == 'MMD':
                ...
                if(self.type != 'third'):
                    dl_rec = torch.distributions.kl_divergence(standard_distribution, ori_distribution)
                if(self.type == 'second'):
                    dl_g = torch.distributions.kl_divergence(view_distribution, tar_distribution)
                    dl_view_0 = torch.distributions.kl_divergence(class_distribution_1, class_distribution_2)
                    dl_view_1 = torch.distributions.kl_divergence(class_distribution_2, class_distribution_3)
                if(self.type == 'third'):
                    dl_rec = torch.distributions.kl_divergence(m_distribution, q_distribution)
...
#                 print(ori_fine,ori_fine.shape)
#                 print(x,x.shape)
                if(self.type == 'first'):
                    loss1, _, _ = calc_cd(ori_coarse_raw, x.permute(0,2,1))
                    loss2, _, _ = calc_cd(ori_coarse_high, x.permute(0,2,1))
                    loss3, _, _ = calc_cd(ori_coarse, x.permute(0,2,1))
                    loss4, _, _ = calc_cd(ori_fine, x.permute(0,2,1))
                    total_train_loss = loss1.mean() * 10 + loss2.mean() * 0.5 + loss3.mean() + loss4.mean() * alpha + dl_rec.mean() * 20
                if(self.type == 'second'):
#                     loss1, _, _ = calc_cd(ori_coarse_raw, x.permute(0,2,1))
#                     loss2, _, _ = calc_cd(ori_coarse_high, x.permute(0,2,1))
#                     loss3, _, _ = calc_cd(ori_coarse, x.permute(0,2,1))
#                     loss4, _, _ = calc_cd(ori_fine, x.permute(0,2,1))
                    loss1, _, _ = calc_cd(ori_coarse_raw, gt)
                    loss2, _, _ = calc_cd(ori_coarse_high, gt)
                    loss3, _, _ = calc_cd(ori_coarse, gt)
                    loss4, _, _ = calc_cd(ori_fine, gt)
                    loss5 = torch.nn.functional.cross_entropy(view_class, label_class)
                    loss6, _, _ = calc_cd(view_raw, torch.cat((ori_coarse_raw,ori_coarse_raw,ori_coarse_raw),0), calc_f1=True)
                    loss7, _, _ = calc_cd(view_high, torch.cat((ori_coarse_high,ori_coarse_high,ori_coarse_high),0), calc_f1=True)
                    loss8, _, _ = calc_cd(view_coarse, torch.cat((ori_coarse,ori_coarse,ori_coarse),0), calc_f1=True)
                    loss9, _, _ = calc_cd(view_fine, torch.cat((ori_fine,ori_fine,ori_fine),0), calc_f1=True)
#                     print('a',loss1.mean())
#                     print('b',loss2.mean())
#                     print('c',loss3.mean())
#                     print('d',loss4.mean())
...
#                     print('j',dl_rec.mean())
#                     print('k',dl_g.mean())
#                     print('l',dl_view_0.mean())
#                     print('m',dl_view_1.mean())

                    
#                     print(dsadsads)
                    
                    total_train_loss = (loss1.mean() * 10 + loss2.mean() * 0.5 + loss3.mean() + loss4.mean() * alpha) * 4 + (dl_view_0.mean() + dl_view_1.mean())* 20 + loss6.mean() * 10 + loss7.mean() * 0.5 + loss8.mean() + loss9.mean() * alpha + loss5.mean()
                    if((torch.isnan(dl_rec.mean()) == False )and (torch.isinf(dl_rec.mean()) == False ) and (torch.isnan(dl_g.mean()) == False ) and (torch.isinf(dl_g.mean()) == False )):
                        total_train_loss += (dl_rec.mean() + dl_g.mean()) * 20
                    print_loss = [loss1.mean(),loss2.mean(),loss3.mean(),loss4.mean(),loss5.mean(),loss6.mean(),loss7.mean(),loss8.mean(),loss9.mean(),dl_rec.mean(),dl_g.mean(),dl_view_0.mean(),dl_view_1.mean()]
                if(self.type == 'third'):
                    loss1, _, _ = calc_cd(ori_coarse_raw, gt)
                    loss2, _, _ = calc_cd(ori_coarse_high, gt)
                    loss3, _, _ = calc_cd(ori_coarse, gt)
                    loss4, _, ff_1 = calc_cd(ori_fine, gt)
                    total_train_loss = loss1.mean() * 10 + loss2.mean() * 0.5 + loss3.mean() + loss4.mean() * alpha
            
                    total_train_loss += (dl_rec.mean()) * 20
            
            else:
                raise NotImplementedError('Only CD is supported')
                
            return ori_fine, loss4, total_train_loss, print_loss
        elif prefix=="val":
            if self.eval_emd:
                emd = calc_emd(fine, gt, eps=0.004, iterations=3000)
            else:
                emd = 0
            cd_p, cd_t, f1 = calc_cd(ori_fine, gt, calc_f1=True)
