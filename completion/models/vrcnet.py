...
#         print(dsadsadsa)
        ...
        if prefix=="train":
            ...
            if self.train_loss == 'cd':
                loss1, _, _ = calc_cd(coarse_raw, gt)
                loss2, _, _ = calc_cd(coarse_high, gt)
                loss3, _, _ = calc_cd(coarse, gt)
                loss4, _, ff_1 = calc_cd(fine, gt)
                print(gt.shape)
                print(fine.shape)
                print(dsadsad)
            else:
            ...
            if((torch.isnan(dl_rec.mean()) == False )and (torch.isinf(dl_rec.mean()) == False ) and (torch.isnan(dl_g.mean()) == False ) and (torch.isinf(dl_g.mean()) == False )):
                ...
                print('1')
#             print('aaa',loss1.mean())
#             print('bbb',loss2.mean())
#             print('ccc',loss3.mean())
#             print('ddd',loss4.mean())
#             print('eee',dl_rec.mean())
#             print('fff',dl_g.mean())
#             print(dsadsadsa)
            if(torch.isnan(total_train_loss) or torch.isinf(total_train_loss)):
                print('aaa',loss1.mean())
                print('bbb',loss2.mean())
                print('ccc',loss3.mean())
                print('ddd',loss4.mean())
            ...
            if self.eval_emd:
                ...
                emd = 0
            cd_pp, cd_tt, f11 = calc_cd(fine, gt, calc_f1=True)
            return {'cd_p': cd_pp, 'cd_t': cd_tt, 'f1': f11}
        elif prefix=="test":
            return {'result': fine}