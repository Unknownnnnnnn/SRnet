...
#     net.eval()
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(dataloader_test):
            ...
            inputs = inputs.transpose(2, 1).contiguous() # 4 2048 3
            
            print(inputs.shape)
            a = inputs[0].cpu().numpy().squeeze()
            b = inputs[1].cpu().numpy().squeeze()
            plot_single_pcd(a,'./res/1.png')
            plot_single_pcd(b,'./res/2.png')
            tar = np.concatenate((a,b),axis = 0)
            plot_single_pcd(tar,'./res/3.png')
            plot_single_pcd(gt[0].cpu().numpy().squeeze(),'./res/4.png')
            print(dsdasdads)
